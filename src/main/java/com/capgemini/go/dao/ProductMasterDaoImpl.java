...
public class ProductMasterDaoImpl implements ProductMasterDao {
	...
	public boolean addProduct(ProductDTO product) throws ProductMasterException, ConnectException {
		...
		try {
			...
			if (resultSet.getInt(1) == 0) {
				 ...
				 * 
				 */
				if (productPrice > 1000000000 || productPrice < 1) {
					throw new ProductMasterException(exceptionProps.getProperty("invalid_price"));
				}
	...
	public boolean updateProduct(ProductDTO product) throws ProductMasterException, ConnectException {
		...
		} catch (DatabaseException | SQLException | IOException | ProductMasterException e) {
			GoLog.logger.error(exceptionProps.getProperty("update_product_failure"));
			throw new ProductMasterException(">>>" + e.getMessage());
	...
	public boolean deleteProduct(String productId) throws ProductMasterException, ConnectException {
		...
		} catch (DatabaseException | ProductMasterException | SQLException | IOException e) {
			throw new ProductMasterException(" >>>" + e.getMessage());
		}
		finally
		{
	...
	public boolean addExistingProduct(ProductDTO product) throws ProductMasterException, ConnectException{
		...
		try {
			...
			statement = connection.prepareStatement(QuerryMapper.PRODUCT_PRESENT);
			statement.setString(1, productID);
			resultSet = statement.executeQuery();
			resultSet.next();
			int prodCount = resultSet.getInt(1);
			if (initialQuantity == 0) {
				...
				for (int index = 1; index <= quantity_to_be_added; index++) {
					...
					preStmt.setString(2, productID + Integer.toString(++prodCount));
					preStmt.setInt(3, 1);
			 ...
			 catch (DatabaseException | IOException |SQLException| ProductMasterException e) {
