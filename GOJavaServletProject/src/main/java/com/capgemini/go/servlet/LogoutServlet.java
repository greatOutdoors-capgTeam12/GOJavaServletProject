...
protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		PrintWriter out =response.getWriter();
			...
			UserDTO currentUser = new UserDTO(null, userId, null, null, 0L, 0,false);
				try {
					result = user.userLogout(currentUser);
				
				if(result)
				{
					((ObjectNode) dataResponse).put("Success :","User Successfully Logged Out");
				}
