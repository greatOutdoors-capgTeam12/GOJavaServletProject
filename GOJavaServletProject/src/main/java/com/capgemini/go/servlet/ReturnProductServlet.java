...
protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    int qty = Integer.parseInt(myMap.get("prodqty"));
		...
		PrintWriter out =response.getWriter();
			...
			SalesRepresentativeService user = new SalesRepresentativeServiceImpl();
				try {
					result = user.returnProduct(orderId,userId,productID,qty,reason);
				
				if(result)
				{
					((ObjectNode) dataResponse).put("Success :","Product Successfully Returned");
				}
