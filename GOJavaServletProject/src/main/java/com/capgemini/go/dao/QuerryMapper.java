...
public class QuerryMapper {
	...
	public static final String ADD_FREQ_PRODUCT_TO_DB = "INSERT INTO `FREQUENTLY_ORDERED_LIST` (USER_ID, PRODUCT_ID)" + "VALUES (?,?)";
	...
	public static final String ORDER_COUNT = "SELECT COUNT(*) FROM `ORDER` WHERE `ORDER_ID` LIKE 'ORD%'";
	...
	public static final String SELECT_REVENUE_BY_MONTH = "SELECT`ORDER_ID`,`USER_ID`,MONTH(`ORDER_INITIATE_TIME`),YEAR(`ORDER_INITIATE_TIME`),`PRODUCT_ID`,`PRODUCT_UIN`,`PRODUCT_PRICE`,`PRODUCT_CATEGORY`FROM`ORDER` INNER JOIN `ORDER_PRODUCT_MAP`USING(ORDER_ID) INNER JOIN `PRODUCT` USING (PRODUCT_ID)";
...
// Address functionality queries	
	...
	public static final String GET_USER_ADDRESSS = "SELECT * FROM `ADDRESS` WHERE `USER_ID` = ? AND `BASE_ADDRESS_STATUS` = 1";
	public static final String PRODUCTS_IN_FAVS = "SELECT `PRODUCT_ID`, COUNT(*) FROM `FREQUENTLY_ORDERED_LIST` WHERE USER_ID=? GROUP BY `PRODUCT_ID` ORDER BY COUNT(*) DESC";
	public static final String VIEW_FAV = "SELECT * FROM `PRODUCT` INNER JOIN `FREQUENTLY_ORDERED_LIST` USING (PRODUCT_ID) WHERE USER_ID = ?"; 
}
