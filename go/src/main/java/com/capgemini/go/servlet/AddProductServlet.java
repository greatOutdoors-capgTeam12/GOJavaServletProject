...
public class AddProductServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			ProductDTO product = new ProductDTO(prodid, Double.parseDouble(price), col, dim, spec, brand, Integer.parseInt(qty), Integer.parseInt(prodcat), prodname);
			ProductMasterService prodmaster = new ProductMasterServiceImpl();
			boolean status = prodmaster.addProduct(product);

			if (status == true) {
				out.println("<div id = \"msg\" class=\"container-fluid \"><p><h2>Product("+prodname +")  Product ID : "
						+ prodid + " has been successfully registered</h2></p></div>");
			}
		} catch (Exception exp) {
			out.println("<div id = \"err\" class=\"container-fluid \"><p><h2>Error in adding Product >> "
